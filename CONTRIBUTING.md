...
1. Update the Rust version used in the Verus CI workflow (`.github/workflows/verus.yml`).
    - Specifically the line `rustup install 1.97.1-x86_64-unknown-linux-gnu` in `.github/workflows/verus.yml`
2. Update the Rust version the VS Code client looks for during installation
    - Specifically the line `const TOOLCHAIN_MAJOR = 97;` in `editors/code/src/bootstrap.ts`
