...
//!
//!
// `inline_function_api` was tightly coupled to a now-removed shape of the
// upstream `inline_call` handler (CallInfo / get_fn_params / inline_simple).
// It has been re-ported against the modern `inline_call` API; the helpers
// it depends on are now `pub(crate)` in `crate::handlers::inline_call`.
//
